...
import com.example.flyweather.db.NewDB;
...
public class ChooseAreaActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initDB();
		if (liteOrm == null) {
			liteOrm = LiteOrm.newSingleInstance(this, "flyWeather.db");
		}
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view, int index,
				...
				if (currentLevel == LEVEL_PROVINCE) {// 当前选中的级别是省
					...
					startActivity(intent);// 开始跳转 finish();
				}
	...
	private void queryProvinces() {
		...
		provinceList = liteOrm.query(Province.class);
		//OrmLog.i("查询全国所有的省", provinceList);
		...
		/*
		 * provinceList = flyWeatherDb.loadProvinces(); if (provinceList.size()
		 * > 0) { dataList.clear(); for (Province province : provinceList) {
		 * dataList.add(province.getProvinceName()); }
		 * adapter.notifyDataSetChanged(); listView.setSelection(0);
		 * titleText.setText("中国"); currentLevel = LEVEL_PROVINCE; } else {
		 * queryFromServer(null, "province"); }
		 */
	}
	...
	private void queryCities() {
		// cityList =
		// flyWeatherDb.loadCities(selectedProvince.getId());//从数据库根据选中的省Id读取某省下所有的城市信息
		QueryBuilder<City> qb =new QueryBuilder<City>(City.class)
				.whereEquals(City.COL_PROVINCEID,selectedProvince.getId());
		//cityList = liteOrm.query(City.class);
		cityList = liteOrm.query(qb);
		if (cityList.size() > 0) {
			dataList.clear();
	...
	private void queryCounties() {
		...
		QueryBuilder<County> qb =new QueryBuilder<County>(County.class)
				.whereEquals(County.COL_CITYID,selectedCity.getId());
		countyList = liteOrm.query(qb);
	...
	private void queryFromServer(final String code, final String type) {
		...
		showProgressDialog();
		final StringRequest request = new StringRequest(address)
...
//									flyWeatherDb, s);
							result = Utility.handleProvincesResponse(s);
						} else if ("city".equals(type)) {
							result = Utility.handleCitiesResponse(s, selectedProvince.getId());
							//HttpUtil.showTips(activity, "选择的省份Id：", selectedProvince.getId()+"");
						} else if ("county".equals(type)) {
							result = Utility.handleCountiesResponse(s, selectedCity.getId());
						}
						if (result) {
		 ...
		 * Utility.handleProvincesResponse(flyWeatherDb, response); } else if
		 * ("city".equals(type)) { result =
		 * Utility.handleCitiesResponse(flyWeatherDb, response,
		 * selectedProvince.getId()); } else if ("county".equals(type)) { result
		 * = Utility.handleCountiesResponse(flyWeatherDb, response,
		 * selectedCity.getId()); } if (result) { //
		 * 通过runOnUiThread()方法回到主线程处理逻辑 runOnUiThread(new Runnable() {
		 * 
		 * @Override public void run() { closeProgressDialog(); if
		 * ("province".equals(type)) { queryProvinces(); } else if
		 * ("city".equals(type)) { queryCities(); } else if
		 * ("county".equals(type)) { queryCounties(); } } }); } }
		 * 
	...
	private void initLiteHttp() {
		if (liteHttp == null) {
			HttpConfig config = new HttpConfig(activity) // configuration
															// quickly
					.setDebugged(true) // log output when debugged
					.setDetectNetwork(true) // detect network before connect
					.setDoStatistics(true) // statistics of time and traffic
					.setUserAgent("Mozilla/5.0 (...)") // set custom User-Agent
					.setTimeOut(10000, 10000); // connect and socket timeout:
												// 10s
	 ...
	 * 共享实例化以后的LiteOrm对象
	 */
	public static LiteOrm shareLiteOrm() {
		return liteOrm;
	}
